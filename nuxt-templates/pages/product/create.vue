...
const execute = () => {
  ...
  formData.append("brand", product.branch);
...
const draftCreate = async () => {
  let formData = execute();
  formData?.append("published", "3");

  const res = await productService.draftCreate(formData);
...
const publishCreate = async () => {
  ...
  const res = await productService.publishCreate(formData);
